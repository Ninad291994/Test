...
SELECT T.ticket_id, strftime('%s','now')- strftime('%s',T.performed_at) AS time, A.status FROM Tickets AS T JOIN Activity AS A ON T.ticket_id = A.ticket_id;



INSERT INTO Ticket_Status_Stats (ticket_id, time_spent_open, time_spent_waiting_on_customer, time_spent_waiting_for_response,
time_till_resolution, time_to_first_response)
SELECT T.ticket_id AS ticket_id, (CASE WHEN T.Status = 'Open' THEN T.time ELSE NULL END) AS time_spent_open, 
